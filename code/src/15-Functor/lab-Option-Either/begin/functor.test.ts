...
describe("Functor", () => {
  ...
  it("Map Option - StrLength & Increment", () => {
    expect(mapOption(strLength)(some(`abcd`))).toEqual(some(4));

    expect(mapOption(increment)(some(4))).toEqual(some(5));
  ...
  it("should get the same result from compose then map and vise versa", () => {
    expect(
      compose(mapOption(strLength), mapOption(increment))(some(`abcd`)),
    ).toEqual(mapOption(compose(strLength, increment))(some(`abcd`)));
  });
  ...
  it("should get the same result from Either then map and vise versa", () => {
    expect(
      compose(mapEither(strLength), mapEither(increment))(right(`abcd`)),
    ).toEqual(mapEither(compose(strLength, increment))(right(`abcd`)));
  });
});
